###
# Functions for Calculating Pi, Theta and Tajima's D 
###
...
#Takes a DIPLOID VCF file and outputs Theta, Pi, and Tajima's D in Windows of size W
#some other scripts included as well for missing data evaluation and reading VCF files
#tested scripts with DNAsptest.fas and with WINDOW1.fas, a file of the first window
...
MinAllele=function(x, ploidy=2){
	#ploidy can be 1 or 2
	if (ploidy==2){
	...
	if(miss>1){
		...
		zero=length(grep("^0[:]",unlist(x)))
		one=length(grep("^1[:]",unlist(x)))
		if(zero>one){return(one)}else{return(zero)}
		}
...
VCF.Pi=function(file=i){
	...
	ones=sapply(x,function(x) (length(x[10:19][grep("^1:", x[st:en])]))) #counts number of "1:'s"
	twos=sapply(x,function(x) (length(x[10:19][grep("^0:", x[st:en])]))) #counts number of "1:'s"
	miss=(abs(twos+ones));miss=abs(miss-((en-st)+1))
	...
	blah[which(blah=="TRUE")]	= (2 * twos[which(blah=="TRUE")])
	blah[which(blah=="FALSE")] =	(2 * ones[which(blah=="FALSE")])
...
VCF.TDThetaPiWindows=function(file=i, w=5000){
	x=Read.VCF(file=i)
	st=grep("FORMAT", unlist(x[1]))+1
	x=x[-1]
	en=length(unlist(x[1]))
	...
	for(site in 1:length(x)){
		#site=1
		ones= length(x[[site]][st:en][grep("^1:", x[[site]][st:en])])
		twos= length(x[[site]][st:en][grep("^0:", x[[site]][st:en])])
		miss=(abs(twos+ones));miss=abs(miss-((en-st)+1))
		miss=((en-st)+1)-miss
